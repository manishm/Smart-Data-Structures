...
public:
	...
	bool read() {
		try {
			...
			int num_read = fscanf(stdin, "%s %d %s %d %s %d %s %d %d %d %d %d %f %d %d %d %d %d %d %d %d %d %d", 
									_alg1_name, &_alg1_num, _alg2_name, &_alg2_num, _alg3_name, &_alg3_num, _alg4_name, &_alg4_num,
					      &_test_no, &_no_of_threads, &_add_ops, &_remove_ops, &_load_factor, &_capacity, &_throughput_time, &_is_dedicated_mode, &_tm_status, &_read_write_delay, &_fc_passes, &_barrier_interval, &_scancount_tuning, &_lock_scheduling, &_dynamic_work_size, &_dynamic_work_intervals );
