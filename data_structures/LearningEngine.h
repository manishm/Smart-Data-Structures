...
class LearningEngine
{
 ...
 private:
   ...
   static volatile _u64 objstatus ATTRIBUTE_CACHE_ALIGNED;
   ...
   void trySpawnHelper(learning_mode_t mode)
   {
      if (mode == disabled)
         return; 

      //cerr << "In trySpawnHelper with mode=" << mode << endl;
   ...
   void tryJoinHelper(learning_mode_t mode)
   {
      if (mode == disabled)
         return;

      //cerr << "In tryJoinHelper with mode=" << mode << endl;
   ...
   void rlupdate() 
   {
      ...
      _u64 start_seq_no = (objstatus & U64(0xFFFFFFFFFFFF));
      _u64 status;
      _u64 num_live;
      _u64 seq_no;
 ...
 public:
   ...
   LearningEngine(unsigned int threads, Monitor *m, learning_mode_t mode = disabled): nthreads(threads), mon(m), mode(mode)
   {
      //cerr << "Instantiating LearningEngine with mode=" << mode << endl;
      //cout << "spawning helper thread" << endl;
      initAPI();
      trySpawnHelper(mode);
      CCP::Memory::read_write_barrier();
      lelistAdd(this);
      FAADD(&objstatus, U64(0x0001000000000001));
      
      //cout << "this = 0x" << (FCIntPtr) this << endl;
   }
   ...
   ~LearningEngine() 
   {
      ...
      CCP::Memory::read_write_barrier();
      lelistRemove(this);
      FAADD(&objstatus, U64(0xFFFF000000000001));
      CCP::Memory::read_write_barrier();
   }
